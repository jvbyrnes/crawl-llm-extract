...
async def main(url: str, output_dir: str = "output", keywords: Optional[str] = None,
               ...
               target_topic: Optional[str] = None, relevance_threshold: float = 0.7) -> None:
    """
    ...
    Args:
        ...
        target_topic: Target topic for LLM-based filtering (e.g., "Python SDK documentation")
    ...
    results = await crawler.crawl_and_parse(url, relevance_threshold)
    ...
    if target_topic:
        print(f"Used LLM filtering for target topic: {target_topic}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target-topic", help="Target topic for LLM-based filtering (e.g., 'Python SDK documentation')")
    parser.add_argument("--relevance-threshold", type=float, default=0.7, help="Minimum relevance score (0.0-1.0) to include page")
